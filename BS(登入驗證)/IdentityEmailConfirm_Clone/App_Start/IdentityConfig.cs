...
using IdentityEmailConfirm_Clone.Models;
using SendGrid;
...
namespace IdentityEmailConfirm_Clone
{
    public class EmailService : IIdentityMessageService
    {
        public async Task SendAsync(IdentityMessage message)
        {
            ...
            await configSendGridasync(message); //async+await
        }

        private async Task configSendGridasync(IdentityMessage message)
        {
            // 將您的電子郵件服務外掛到這裡以傳送電子郵件。
            var apiKey = "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";
